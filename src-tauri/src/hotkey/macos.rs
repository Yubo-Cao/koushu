...
fn key_code_for(name: &str) -> Option<u16> {
    Some(match name {
        ...
        "3" => 20,
        "4" => 21,
        // 5 and 6 are out of order in the header, and so are 7/8/9. Copied as
        // published rather than tidied, because the layout is the hardware's.
        "5" => 23,
        "6" => 22,
        "7" => 26,
        "8" => 28,
    ...
    F: Fn(PttEdge) + Send + Sync + 'static,
{
    ...
    Ok((
        HotkeyStatus {
            ...
            // The tap sees every key, so the chord that was asked for is the
            // chord that is matched. Nothing can refuse it once Accessibility
            // is granted, and that is checked above.
            ok: true,
