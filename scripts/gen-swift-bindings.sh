...
LIB="${ROOT_DIR}/target/release/libkoushu_core.dylib"
[ -f "${LIB}" ] || LIB="${ROOT_DIR}/target/release/libkoushu_core.so"
if [ ! -f "${LIB}" ]; then
  echo "no cdylib at target/release/libkoushu_core.{dylib,so}" >&2
  exit 1
fi

rm -rf "${OUT_DIR}"
mkdir -p "${OUT_DIR}"
cargo run -p koushu-core --bin uniffi-bindgen -- \
